...
const InstallationTypeEdit: React.FC<FormModalProps> = ({ modalClose }) => {
    const {
        control,
        register,
        handleSubmit,
    ...
    const updateCategory = useUpdateData([`installationsType`], `/v1/installation-types/update-installation-type/${editData._id}`, token)
    const onSubmit = (data: any) => {
        try {
            updateCategory.mutate(data, {
                onSuccess: (responseData) => {
                    toast.success("Successfully updated installation type!");
                    reset();
                    modalClose(false)
                },
                onError: (error: any) => {
    ...
    return (
        ...
        >
            ...
            <h2 className="text-xl font-semibold text-primary">Edit Installation Type</h2>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                ...
                <Input
                    ...
                    name="name"
                    type="text"
                    placeholder="Enter installation type name"
                    register={register}
                    error={errors.name}
                    required
            ...
            <FormSubmitButton
                ...
                buttonName="Update Installation Type"
                context="Updating"
            />
        </form>
    );
...
export default InstallationTypeEdit;
